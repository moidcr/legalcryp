...
$langsLoad=array('projects', 'bills', 'orders', "salaries");
...
$objectsal = new Salary($db);
if ($id > 0 || !empty($ref)) {
	$childids = $user->getAllChildIds(1);
	$objectsal->fetch($id, $ref);
	...
	if (!empty($user->rights->salaries->read) && $objectsal->fk_user > 0 && in_array($objectsal->fk_user, $childids)) {
		$canread = 1;
	}
	if (!$canread) {
		accessforbidden();
...
$head = salaries_prepare_head($objectsal);
...
//if (($id > 0 || !empty($ref)) || $projectidforalltimes > 0) {
	...
	$linktocreatetime = '';
		...
		print '<form id="frmspendsalary" method="POST" action="timespenttosalaryrelated.php">';
		if ($optioncss != '') {
		...
		$sql .= " JOIN timespenttosalary_reg tr on t.rowid = tr.timespent_id AND tr.salary_id = '$id'";
		...
		print dol_get_fiche_head($head, 'spendsrelated', $langs->trans("SalaryPayment"), -1, 'salary');	
		...
		$userstaticg = new User($db);
		$userstaticg->fetch($objectsal->fk_user);

		$morehtmlref .= $langs->trans('Employee').' : '.$userstaticg->getNomUrl(1);
		$morehtmlref .= '</div>';

		dol_banner_tab($objectsal, 'id', $linkback, 1, 'rowid', 'ref', $morehtmlref, '', 0, '', '');
		...
		*/
		...
		if (!empty($arrayfields['author']['checked'])) {
			...
			print_liste_field_titre($arrayfields['author']['label'], $_SERVER['PHP_SELF'], 'u.lastname', '', $param, '', $sortfield, $sortord);
		}
