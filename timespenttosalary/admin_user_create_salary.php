...
$contextpage = GETPOST('contextpage', 'aZ') ?GETPOST('contextpage', 'aZ') : 'employeelist'; // To manage different context of search
...
print '<input type="hidden" name="typec" value="user">';
...
if (!empty($arrayfields['u.login']['checked'])) {
	print_liste_field_titre("Login", $_SERVER['PHP_SELF'], "u.login", $param, "", 'id="Titlelogin"', $sortfield, $sortorder);
}
if (!empty($arrayfields['u.lastname']['checked'])) {
...
while ($i < ($limit ? min($num, $limit) : $num)) {
	...
	if (!empty($arrayfields['u.login']['checked'])) {
		...
		$salH_ = (($obj->salary) && $obj->salary != "") ? $obj->salary : "0.00";
		print '<input id="spendu'.$i.'" name="spendu[]" type="checkbox" value="'. $obj->rowid.'|'.$salH_.'"  /> ';
		print $li;
...
<script>
	...
	if(cn > 0)
	{
		$("#searchFormList").prop( "action", "salary.php");
	...
	$( document ).ready(function() {
		$("#checkforselects").hide();
		if($("#Titlelogin").length>0)
		{
			$("#Titlelogin").prepend("<input id=\"CheckAll\" onclick=\"CheckedSAll()\" type=\"checkbox\" value=\"\"  /> ");
		}
	});
